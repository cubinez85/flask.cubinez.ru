...
from app.models import User, Post, Tree, Message, Notification
...
def tree():
    ...
    cursor = db.session.execute(text('select * from tree'))
    content = cursor.fetchall()
    cursor = db.session.execute(text('select * from tree'))
    labels = cursor.fetchall()
    labels = list(map(lambda x: x[0:], cursor.keys()))

    return render_template('tree.html', labels=labels, content=content)
