...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def verify_reset_password_token(token):
        s = Serializer(current_app.config['SECRET_KEY'])
        try:
            data = s.loads(token, max_age=600)  # Проверка срока действия (600 секунд)
            user_id = data.get('user_id')
        except BadSignature:
            # Токен недействителен (подпись не совпадает или истек)
            return None
        except Exception as e:
            # Другая ошибка при десериализации
        ...
        user = db.session.get(User, user_id) # Используйте db.session.get вместо User.query.get
        return user
...
admin.add_view(TreeAdmin(Tree, db.session))
