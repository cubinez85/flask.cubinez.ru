...
from app import db, login, admin, bcrypt
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    push_subscription: so.Mapped[Optional[str]] = so.mapped_column(sa.Text, nullable=True)
...
class Post(SearchableMixin, db.Model):
    ...
    # Поля для файлов в постах
    filename: so.Mapped[Optional[str]] = so.mapped_column(sa.String(255), nullable=True)
    file_path: so.Mapped[Optional[str]] = so.mapped_column(sa.String(500), nullable=True)
    file_size: so.Mapped[Optional[int]] = so.mapped_column(nullable=True)
    file_mimetype: so.Mapped[Optional[str]] = so.mapped_column(sa.String(100), nullable=True)
...
class Employee(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    parent: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('employee.id'), nullable=True)
    name: so.Mapped[str] = so.mapped_column(sa.String(64))
    position: so.Mapped[str] = so.mapped_column(sa.String(64))

    # Связь для подчиненных
    children: so.WriteOnlyMapped['Employee'] = so.relationship(
        'Employee',
    ...
    # Связь для родителя
    parent_ref: so.Mapped[Optional['Employee']] = so.relationship(
        'Employee',
        remote_side=[id],
...
# ДОБАВЛЕННЫЙ КЛАСС Follow
class Follow(db.Model):
    __tablename__ = 'follows'
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    follower_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(User.id, ondelete='CASCADE'))
    followed_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(User.id, ondelete='CASCADE'))
    timestamp: so.Mapped[datetime] = so.mapped_column(default=lambda: datetime.now(timezone.utc))

    # Relationships
    follower: so.Mapped[User] = so.relationship(foreign_keys=[follower_id], backref='followed_relationships')
    followed: so.Mapped[User] = so.relationship(foreign_keys=[followed_id], backref='follower_relationships')
...
class PostAdmin(SecureModelView):
    column_list = ('body', 'timestamp')

class EmployeeAdmin(SecureModelView):
    column_list = ('id', 'parent', 'name', 'position')
    form_columns = ['parent', 'name', 'position']

# ДОБАВЛЕННЫЙ КЛАСС ДЛЯ АДМИН-ПАНЕЛИ Follow
